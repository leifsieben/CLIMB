...
Each tick is a REFERENCE feature block; the reference itself is the zero line, and every bar is
the percentage lift one addition buys on top of it, signed so positive is always better. The same
XGBoost, the same scaffold 5-fold CV, and every cell from one environment --- the RDKit arm was
regenerated alongside Mordred so that a family difference changes only the descriptor set.
Descriptors are demonstrably orthogonal to fingerprints, which is the positive control that makes
a flat embedding bar readable: adding descriptors to ECFP4 gains $1.4$--$13.2\%$ across panels
(BACE alone is flat). Both CLIMB embeddings are redundant given classical features --- negative on
most references --- while CheMeleon carries real information beyond ECFP4 ($0$--$8.2\%$).
The Mordred tick is the sharp test, because CheMeleon is a D-MPNN pretrained to regress the
$1{,}613$ Mordred 2D descriptors: on five of six panels its lift over Mordred lies within
$\pm0.6\%$, i.e. it has compressed its pretraining target rather than learned past it.
MoleculeACE is the exception at $+3.6\%$ and is the one panel where that reading does not hold.
Intervals are pending: the uncertainty on a lift is the spread of the per-fold DIFFERENCE, in
which fold difficulty cancels, and the aggregate tables report only each arm's marginal SD ---
which runs $2$--$8\times$ the lift and describes a different quantity.}
