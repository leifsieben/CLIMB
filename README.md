...
## Data, models & reproduction

All artifacts are on the Hugging Face Hub in three repositories, currently **private** with access
granted to reviewers (they flip public on release):

| Artifact | What | Hugging Face |
|---|---|---|
| **Checkpoints** | the ~41M-param ModernBERT encoder for every run, plus the byte-BPE tokenizer (vocab 1000) | [`lsieben/climb-encoders`](https://huggingface.co/lsieben/climb-encoders) |
| **Raw results** | per-model eval outputs — 5-fold-CV + hold-out `suite_summary.json`, per-molecule `test_predictions.csv`, `metrics.jsonl` | [`lsieben/climb-results`](https://huggingface.co/datasets/lsieben/climb-results) |
| **Pre-training data** | tokenized PubChem MLM/MTR corpus, 217-descriptor targets, supervised SFT table, eval-leakage blocklist | [`lsieben/climb-pretrain-data`](https://huggingface.co/datasets/lsieben/climb-pretrain-data) |

**To reproduce any figure or table, follow [`REPRODUCE.md`](REPRODUCE.md)** — it maps every
figure → notebook cell → data → exact command. Figures are generated by `climb_figures.ipynb` from the
raw results; `figure_data_manifest.json` fingerprints the exact results snapshot, so a reviewer can
verify their download byte-for-byte with `python scripts/verify_notebook_sync.py`.

### Release & working status (2026-08-05)

- **Published:** all three HF repos are live (private): `lsieben/climb-encoders` (122 checkpoints),
  `lsieben/climb-results` (raw per-run eval), `lsieben/climb-pretrain-data` (12M corpus + descriptors
  + supervised table + blocklist). The 124M superset is **linked**, not re-hosted
  (`hheiden/PubChem-124M-SMILES-SELFIES-InChI-IUPAC`, §6.1b). GitHub `main` is current.
- **Recently landed (compute session):** two-corpus scaling documentation (§3, §6.1b — the 50M/100M
  rungs and their 124M corpus); the SI vocabulary-size sweep (§7.2, Fig S2); and a **statistics-rigor
...
⚠️ Counts are **as loaded by the DeepChem/RDKit versions pinned here** (§9.1c) and can vary by a few
