...
All artifacts are public on the Hugging Face Hub, in three repositories:

| Artifact | What | Hugging Face |
|---|---|---|
| **Checkpoints** | the ~41M-param ModernBERT encoder for every run, plus the byte-BPE tokenizer (vocab 1000) | [`<org>/climb-encoders`](https://huggingface.co/<org>/climb-encoders) |
| **Raw results** | per-model eval outputs — 5-fold-CV + hold-out `suite_summary.json`, per-molecule `test_predictions.csv`, `metrics.jsonl` | [`<org>/climb-results`](https://huggingface.co/datasets/<org>/climb-results) |
| **Pre-training data** | tokenized PubChem MLM/MTR corpus, 217-descriptor targets, supervised SFT table, eval-leakage blocklist | [`<org>/climb-pretrain-data`](https://huggingface.co/datasets/<org>/climb-pretrain-data) |
...
Public on the Hugging Face Hub (three repos; see the availability table at the top of this README).
