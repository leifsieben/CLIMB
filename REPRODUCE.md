...
Every figure in the paper is generated by a script in [`figures/`](figures/) from the eval outputs in
`figure_data/`, which are in turn produced by `eval_v2.py` from the model checkpoints. See
[`figures/README.md`](figures/README.md) for that pipeline.

> The cell-by-cell mapping later in this file was written against `climb_figures.ipynb`, which the
> script pipeline replaced. The artifact download, evaluation and training sections below are
> current; the notebook-cell references are historical and are being rewritten.
...
| Artifact | Hugging Face repo | Place it at |
|---|---|---|
...
> All three repos are public; no login or access request is needed.
